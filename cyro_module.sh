...
check_update() {
  ...
  LATEST=$($CURL -sk $GH_RESOLVE     "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/cyro_version.txt" 2>/dev/null)
  [ -z "$LATEST" ] && echo "[*] Tidak bisa cek update, lanjut..." && return
  if [ "$LATEST" != "$CURRENT_VERSION" ]; then
    echo "[*] Ada update! v$CURRENT_VERSION -> v$LATEST"
    $CURL -sk $GH_RESOLVE       "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/cyro_module.sh"       -o "$SCRIPT_PATH.new"
...
wait_approval() {
  ...
  while [ $attempts -lt $max ]; do
    ...
    if [ -n "$approve" ]; then
      ...
      tg_send "$ADMIN_ID" "APPROVED! Serial: $SERIAL Module: Sensix Cyro"
      return 0
    elif [ -n "$blacklist" ]; then
      save_status_local "blacklisted"
      echo "$SERIAL|cyro|blacklisted" >> "$DB_FILE"
      tg_send "$ADMIN_ID" "BLACKLISTED! Serial: $SERIAL Module: Sensix Cyro"
      return 1
    fi
  done
