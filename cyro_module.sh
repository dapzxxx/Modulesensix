...
check_update() {
  ...
  LATEST=$($CURL -sk $GH_RESOLVE \
    "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/cyro_version.txt" 2>/dev/null)
  [ -z "$LATEST" ] && echo "[*] Tidak bisa cek update, lanjut..." && return
  if [ "$LATEST" != "$CURRENT_VERSION" ]; then
    echo "[*] Ada update! v$CURRENT_VERSION -> v$LATEST"
    $CURL -sk $GH_RESOLVE \
      "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/cyro_module.sh" \
      -o "$SCRIPT_PATH.new"
...
check_db() {
  grep "^$1|cyro|" "$DB_FILE" 2>/dev/null | cut -d"|" -f3
}

notify_new_device() {
  TEXT="🔔 <b>DEVICE BARU!</b>
🎯 <b>Module  :</b> Sensix Cyro
...
⏳ Menunggu persetujuan..."
  ...
  KB=$(printf '{"inline_keyboard":[[{"text":"✅ APPROVE","callback_data":"approve_%s_cyro"},{"text":"🚫 BLACKLIST","callback_data":"blacklist_%s_cyro"}]]}' "$SERIAL" "$SERIAL")
...
},{"text":"🚫 BLACKLIST","callback_data":"blacklist_'"'"'$SERIAL'"'"'_cyro"}]]}'
...
wait_approval() {
  ...
  while [ $attempts -lt $max ]; do
    ...
    approve=$(echo "$updates" | grep -o '"data":"approve_'"$SERIAL"'_cyro"')
    blacklist=$(echo "$updates" | grep -o '"data":"blacklist_'"$SERIAL"'_cyro"')
    if [ -n "$approve" ]; then
      save_status_local "approved"
      echo "$SERIAL|cyro|approved" >> "$DB_FILE"
      tg_send "$ADMIN_ID" "✅ <code>$SERIAL</code> di-APPROVE untuk Sensix Cyro!"
      return 0
    elif [ -n "$blacklist" ]; then
      save_status_local "blacklisted"
      echo "$SERIAL|cyro|blacklisted" >> "$DB_FILE"
      tg_send "$ADMIN_ID" "🚫 <code>$SERIAL</code> di-BLACKLIST untuk Sensix Cyro!"
      return 1
    fi
  done
...
run_module() {
  ...
  echo "║  Module Name  : Sensix Cyro    ║"
  echo "║  Version      : $CURRENT_VERSION              ║"
  echo "╚════════════════════════════════╝"
  echo ""
  if [ -f "/sdcard/Cyro/cyro.sh" ]; then
    sh /sdcard/Cyro/cyro.sh "$@"
  else
    echo "[!] cyro.sh tidak ditemukan di /sdcard/Cyro/"
  fi
}
