#!/system/bin/sh
# ============================================================
#   MODULESENSIX BOT LISTENER - Fixed Parser
...
# Ambil nilai string dari JSON key
parse_json() {
  echo "$1" | grep -o "\"${2}\":\"[^\"]*\"" | head -1 | cut -d'"' -f4
}

# Ambil nilai integer dari JSON key  
parse_json_int() {
...
handle_cmd() {
  ...
  echo "[CMD] cmd=$cmd arg1=$arg1 arg2=$arg2 chat=$chat"
...
/blacklist ABC123 void"
    ...
    /reset)
      ...
      ;;
    /status)
      text_out="📊 <b>Status <code>${arg1}</code>:</b>\n\n"
      for m in void cyro nexa xron; do
        st=$(check_db "$arg1" "$m")
        [ -z "$st" ] && st="belum terdaftar" && icon="❌"
        [ "$st" = "approved" ] && icon="✅"
        [ "$st" = "blacklisted" ] && icon="🚫"
        text_out="${text_out}${icon} ${m}: ${st}\n"
      done
      send_msg "$chat" "$text_out"
      ;;
    /listdevice)
      if [ ! -s "$DB_FILE" ]; then
        send_msg "$chat" "📭 Belum ada device terdaftar."
      else
        text_out="📋 <b>DAFTAR DEVICE:</b>\n\n"
        while IFS='|' read -r s m st; do
          [ "$st" = "approved" ] && icon="✅" || icon="🚫"
          text_out="${text_out}${icon} <code>${s}</code> | ${m} - ${st}\n"
        done < "$DB_FILE"
        send_msg "$chat" "$text_out"
      fi
      ;;
...
# Reset offset biar mulai dari awal
echo "0" > "$OFFSET_FILE"
offset=0
...
while true; do
  ...
  if echo "$response" | grep -q '"update_id"'; then
    ...
    # Ambil semua update_id
    for uid in $(echo "$response" | grep -o '"update_id":[0-9]*' | cut -d: -f2); do
      ...
      if echo "$response" | grep -q '"callback_query"'; then
        ...
        if [ "$from_id" = "$ADMIN_ID" ] && [ -n "$cb_data" ]; then
          echo "[CB] data=$cb_data"
          handle_callback "$cb_id" "$cb_data" "$ADMIN_ID"
        fi

      else
        # Parse pesan - gunakan format JSON yang konsisten
        # from id ada di "from":{"id":NUMBER
        from_id=$(echo "$response" | grep -o '"from":{"id":[0-9]*' | head -1 | grep -o '[0-9]*$')
        # chat id ada di "chat":{"id":NUMBER
        chat_id=$(echo "$response" | grep -o '"chat":{"id":[0-9-]*' | head -1 | grep -o '[0-9-]*$')
        # text ada di "text":"VALUE"
