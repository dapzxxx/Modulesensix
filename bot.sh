#!/system/bin/sh
# ============================================================
#   MODULESENSIX BOT LISTENER - Pure Shell + curl
...
save_db() {
  ...
  local module="$2"
  local status="$3"
  grep -v "^${serial}|${module}|" "$DB_FILE" > /tmp/.db_tmp 2>/dev/null
  echo "${serial}|${module}|${status}" >> /tmp/.db_tmp
  cp /tmp/.db_tmp "$DB_FILE"
  echo "[DB] ${serial} | ${module} => ${status}"
}
...
handle_cmd() {
  ...
  case "$cmd" in
    "/approve")
      if [ -z "$module" ]; then
        # Approve semua module
        save_db "$serial" "all" "approved"
        send_msg "$chat" "✅ Device <code>${serial}</code> di-<b>APPROVE</b> semua module!"
      else
        save_db "$serial" "$module" "approved"
        send_msg "$chat" "✅ Device <code>${serial}</code> di-<b>APPROVE</b> untuk module <b>${module}</b>!"
      fi
      ;;
    "/blacklist")
      if [ -z "$module" ]; then
        save_db "$serial" "all" "blacklisted"
        send_msg "$chat" "🚫 Device <code>${serial}</code> di-<b>BLACKLIST</b> semua module!"
      else
        save_db "$serial" "$module" "blacklisted"
        send_msg "$chat" "🚫 Device <code>${serial}</code> di-<b>BLACKLIST</b> untuk module <b>${module}</b>!"
      fi
      ;;
    "/reset")
      grep -v "^${serial}|" "$DB_FILE" > /tmp/.db_tmp 2>/dev/null
      cp /tmp/.db_tmp "$DB_FILE"
      send_msg "$chat" "🔄 Device <code>${serial}</code> di-<b>RESET</b>! Perlu approve ulang."
      ;;
    "/addmodule")
      save_db "$serial" "$module" "approved"
      send_msg "$chat" "➕ Module <b>${module}</b> ditambahkan untuk <code>${serial}</code>!"
      ;;
    "/status")
      text="📊 <b>Status device <code>${serial}</code>:</b>\n\n"
      for m in void cyro nexa xron; do
        st=$(check_db "$serial" "$m")
        [ -z "$st" ] && st="belum terdaftar"
        [ "$st" = "approved" ] && icon="✅" || icon="❌"
        [ "$st" = "blacklisted" ] && icon="🚫"
        text="${text}${icon} <b>${m}</b>: ${st}\n"
      done
      send_msg "$chat" "$text"
      ;;
    "/listdevice")
      if [ ! -s "$DB_FILE" ]; then
        ...
        while IFS='|' read -r s m st; do
          [ "$st" = "approved" ] && icon="✅" || icon="🚫"
          text="${text}${icon} <code>${s}</code> | <b>${m}</b> - ${st}\n"
        done < "$DB_FILE"
        send_msg "$chat" "$text"
      fi
      ;;
...
<b>Contoh:</b>
/approve ABC123 void
/blacklist ABC123 cyro
/approve ABC123 (semua module)"
      ;;
  esac
}
  ...
  if echo "$response" | grep -q '"update_id"'; then
    ...
    for uid in $update_ids; do
      ...
      if echo "$response" | grep -q '"callback_query"'; then
        ...
        cb_id=$(echo "$response" | grep -o '"callback_query":{"id":"[^"]*"' | cut -d'"' -f4)
        ...
        if [ "$from_id" = "$ADMIN_ID" ] && [ -n "$msg_text" ]; then
          ...
          echo "[MSG] $cmd $arg1 $arg2"
          handle_cmd "$cmd" "$arg1" "$arg2" "$chat_id"
        fi
      fi
    done
  fi
