...
check_update() {
  ...
  LATEST=$($CURL -sk $GH_RESOLVE \
    "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/void_version.txt" 2>/dev/null)
  [ -z "$LATEST" ] && echo "[*] Tidak bisa cek update, lanjut..." && return
  if [ "$LATEST" != "$CURRENT_VERSION" ]; then
    echo "[*] Ada update! v$CURRENT_VERSION -> v$LATEST"
    $CURL -sk $GH_RESOLVE \
      "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/void_module.sh" \
      -o "$SCRIPT_PATH.new"
...
⏳ Menunggu persetujuan..."
  KB='{"inline_keyboard":[[{"text":"✅ APPROVE","callback_data":"approve_'"'"'$SERIAL'"'"'_void"},{"text":"🚫 BLACKLIST","callback_data":"blacklist_'"'"'$SERIAL'"'"'_void"}]]}'
...
wait_approval() {
  ...
  while [ $attempts -lt $max ]; do
    ...
    if [ -n "$approve" ]; then
      ...
      tg_send "$ADMIN_ID" "✅ <code>$SERIAL</code> di-APPROVE untuk Sensix Void!"
      return 0
    elif [ -n "$blacklist" ]; then
      save_status_local "blacklisted"
      echo "$SERIAL|void|blacklisted" >> "$DB_FILE"
      tg_send "$ADMIN_ID" "🚫 <code>$SERIAL</code> di-BLACKLIST untuk Sensix Void!"
      return 1
    fi
  done
