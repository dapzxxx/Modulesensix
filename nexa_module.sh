...
check_update() {
  ...
  LATEST=$($CURL -sk $GH_RESOLVE \
    "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/nexa_version.txt" 2>/dev/null)
  [ -z "$LATEST" ] && echo "[*] Tidak bisa cek update, lanjut..." && return
  if [ "$LATEST" != "$CURRENT_VERSION" ]; then
    echo "[*] Ada update! v$CURRENT_VERSION -> v$LATEST"
    $CURL -sk $GH_RESOLVE \
      "https://raw.githubusercontent.com/$GITHUB_USER/$GITHUB_REPO/main/nexa_module.sh" \
      -o "$SCRIPT_PATH.new"
...
check_db() {
  grep "^$1|nexa|" "$DB_FILE" 2>/dev/null | cut -d"|" -f3
}

notify_new_device() {
  TEXT="🔔 <b>DEVICE BARU!</b>
🎯 <b>Module  :</b> Sensix Nexa
...
⏳ Menunggu persetujuan..."
  ...
  KB=$(printf '{"inline_keyboard":[[{"text":"✅ APPROVE","callback_data":"approve_%s_nexa"},{"text":"🚫 BLACKLIST","callback_data":"blacklist_%s_nexa"}]]}' "$SERIAL" "$SERIAL")
...
},{"text":"🚫 BLACKLIST","callback_data":"blacklist_'"'"'$SERIAL'"'"'_nexa"}]]}'
...
wait_approval() {
  ...
  while [ $attempts -lt $max ]; do
    ...
    approve=$(echo "$updates" | grep -o '"data":"approve_'"$SERIAL"'_nexa"')
    blacklist=$(echo "$updates" | grep -o '"data":"blacklist_'"$SERIAL"'_nexa"')
    if [ -n "$approve" ]; then
      save_status_local "approved"
      echo "$SERIAL|nexa|approved" >> "$DB_FILE"
      tg_send "$ADMIN_ID" "✅ <code>$SERIAL</code> di-APPROVE untuk Sensix Nexa!"
      return 0
    elif [ -n "$blacklist" ]; then
      save_status_local "blacklisted"
      echo "$SERIAL|nexa|blacklisted" >> "$DB_FILE"
      tg_send "$ADMIN_ID" "🚫 <code>$SERIAL</code> di-BLACKLIST untuk Sensix Nexa!"
      return 1
    fi
  done
...
run_module() {
  ...
  echo "║  Module Name  : Sensix Nexa    ║"
  echo "║  Version      : $CURRENT_VERSION              ║"
  echo "╚════════════════════════════════╝"
  echo ""
  if [ -f "/sdcard/Nexa/nexa.sh" ]; then
    sh /sdcard/Nexa/nexa.sh "$@"
  else
    echo "[!] nexa.sh tidak ditemukan di /sdcard/Nexa/"
  fi
}
